...
mod token {
    ...
    mod tests {
        ...
        type Event = <TokenInterface as ::ink_lang::BaseEvent>::Type;
        ...
        fn assert_transfer_event(
            ...
            let expected_topics = vec![
                encoded_into_hash(&PrefixedValue {
                    value: b"TokenInterface::Transfer",
                    prefix: b"",
                }),
                encoded_into_hash(&PrefixedValue {
                    prefix: b"TokenInterface::Transfer::from",
                    value: &expected_from,
                }),
                encoded_into_hash(&PrefixedValue {
                    prefix: b"TokenInterface::Transfer::to",
                    value: &expected_to,
                }),
                encoded_into_hash(&PrefixedValue {
                    prefix: b"TokenInterface::Transfer::value",
        ...
        fn new_works() {
            // Constructor works.
            let _erc20 = TokenInterface::new(100);
        ...
        fn total_supply_works() {
            // Constructor works.
            let token = TokenInterface::new(100);
            // Transfer event triggered during initial construction.
            let emitted_events = ink_env::test::recorded_events().collect::<Vec<_>>();
            assert_transfer_event(
                &emitted_events[0],
        ...
        fn transfer_works() {
            // Constructor works.
            let mut token = TokenInterface::new(100);
            // Transfer event triggered during initial construction.
            let accounts =
                ink_env::test::default_accounts::<ink_env::DefaultEnvironment>()
                    .expect("Cannot get accounts");
            ...
            assert_eq!(token.transfer(accounts.bob, 10), Ok(()));
            // Bob owns 10 tokens.
        ...
        fn invalid_transfer_should_fail() {
            ...
            assert_eq!(
                token.transfer(accounts.eve, 10),
        ...
        fn transfer_from_works() {
            ...
            assert_eq!(
                token.transfer_from(accounts.alice, accounts.eve, 10),
                Err(Error::InsufficientAllowance)
            );
            // Alice approves Bob for token transfers on her behalf.
            assert_eq!(token.approve(accounts.bob, 10), Ok(()));
            ...
            // Eve owns tokens.
            assert_eq!(token.balance_of(accounts.eve), 10);
        ...
        fn allowance_must_not_change_on_failed_transfer() {
            ...
            assert_eq!(token.approve(accounts.bob, initial_allowance), Ok(()));
            ...
            assert_eq!(
                token.transfer_from(accounts.alice, accounts.eve, alice_balance + 1),
                Err(Error::InsufficientBalance)
            );
